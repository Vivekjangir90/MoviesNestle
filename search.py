...
def app():
    ...
    sercher = st.text_input("Search Here ","")
    url = "https://api.themoviedb.org/3/search/multi?api_key=1b46d8d88c680c018f026ea039d36f98&query={}&include_adult=false&language=en-US&page={}".format(sercher,st.session_state.page_number)

    response = requests.get(url)
    response = response.json()


    title_m = [(item.get("name") or item.get("title","null")) for item in response.get("results", []) if (item.get("name") or item.get("title")) and item.get("poster_path")]
    post_m = [item.get("poster_path") or "null" for item in response.get('results',[]) if (item.get("name") or item.get("title")) and item.get("poster_path")]
